...
Bdxyz,Bdyzx,Bdzxy = np.gradient(fullfield(XYZ,YZX,ZXY))


# Interpolate the gradient of the fullfield
Bdxfn = RegularGridInterpolator((x,y,z), Bdxyz*1e4,bounds_error=False,fill_value=0)
Bdyfn = RegularGridInterpolator((x,y,z), Bdyzx*1e4,bounds_error=False,fill_value=0)
Bdzfn = RegularGridInterpolator((x,y,z), Bdzxy*1e4,bounds_error=False,fill_value=0)

#%%
zarr = np.arange(30)*mm
...
def alp(zp):
    if (zp >= 10*mm and zp < 20*mm) or (zp >= 30*mm and zp <= 40*mm):
        alpha = alpSFS
    elif (zp >40*mm):
        alpha = 0
    else:
...
# Presets
    # tlast = 1000*um; v0 = 0,0,60
    # tlast = 3800*um; v0 = 0,0,13
...
def equation_system(b,t):
    ...
    db_over_dt = [vxt,vyt,vzt, alp(xt,yt,zt)*Bdxfn([xt,yt,zt]) , alp(xt,yt,zt)*Bdyfn([xt,yt,zt]), alp(xt,yt,zt)*Bdzfn([xt,yt,zt])]
