...
for i in range(36):
    ...
    inner_points_xz = plot_surface_matrix(surface_plots_xz, i)
    energy_curves.append(Energy_Curve(inner_points_xz, \
    "Ground State Energy Curve " + str(i + 1)))
    ax.plot_surface(XZ, ZX, inner_points_xz)
plt.show()
...
e_XZ, e_ZX = np.meshgrid(x, z, indexing='ij', sparse=True)
...
for i in range(24):
    ...
    excited_inner_points_xz = plot_surface_matrix(surface_plots_xz, i)
    excited_energy_curves.append(Energy_Curve(excited_inner_points_xz, \
    "Excited Energy Curve" + str(i + 1)))
    ax.plot_surface(e_XZ, e_ZX, excited_inner_points_xz)
plt.show()
...
ax.plot_surface(XZ, ZX, excited_energy_curves[10].energy_values)
ax.plot_surface(XZ, ZX, excited_energy_curves[20].energy_values)
